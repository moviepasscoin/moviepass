...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("moviepass"))
        return false;
...
{
    // Convert moviepass:// to moviepass:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("moviepass://"))
    {
        uri.replace(0, 12, "moviepass:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MoviePass.lnk";
}
...
{
    return GetAutostartDir() / "moviepass.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MoviePass-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  moviepass-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MoviePass-Qt"));

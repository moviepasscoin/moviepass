...
#else
    const char* pszModule = "moviepass";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoviePass
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoviePass
    // Mac: ~/Library/Application Support/MoviePass
    // Unix: ~/.moviepass
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoviePass";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "moviepass.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "moviepassd.pid"));
